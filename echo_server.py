...
def server(log_buffer=sys.stderr):
    ...
    print("SERVER        making a server on {0}:{1}".format(*address), file=log_buffer, flush=True)
    ...
    try:
        ...
        while True:
            print('SERVER        waiting for a connection', file=log_buffer, flush=True)
            ...
            try:
                print('SERVER        connection from - {0}:{1}'.format(*addr), file=log_buffer, flush=True)
                ...
                while True:
                    ...
                    #       formatting
                    data = conn.recv(16) # server
                    print('SERVER        received "{0}"'.format(data.decode('utf8')), file=log_buffer, flush=True)
                    
                    if data.decode('utf8') == '****shutdown****':
                    ...
                    # debugging problems.
                    conn.sendall(data)
                    print('SERVER        sent     "{0}"'.format(data.decode('utf8')), file=log_buffer, flush=True)
                    
                    # TODO: Check here to see whether you have received the end
                    # of the message. If you have, then break from the `while True`
                    # loop.
                    if len(data) < 16:
                        print("SERVER        message length is less than 16", file=log_buffer, flush=True)
                        # conn.shutdown()
                        break
                ...
                print('SERVER        conn.shutdown', file=log_buffer, flush=True)
                conn.shutdown(socket.SHUT_RDWR)
                print('SERVER        conn.close', file=log_buffer, flush=True)
                conn.close()
                print('SERVER        echo complete', file=log_buffer, flush=True)
        ...
        # print('SERVER        sock.shutdown', file=log_buffer, flush=True)
        # sock.shutdown(socket.SHUT_RDWR)
        print('SERVER        sock.close', file=log_buffer, flush=True)
        sock.close()
